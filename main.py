...
CHINESE_STOPWORDS = {
    '的', '了', '和', '是', '在', '就', '都', '而', '及', '與', '著', '或', '一個', '沒有',
    '我們', '你們', '他們', '是否', '可以', '以及', '因為', '所以', '如果', '這個', '那個',
    '不是', '使用', '進行', '一樣', '其中', '自己', '比較', '這樣', '一種', '來說', '之後',
    '比如說', '什麼', '如何', '有點', '不能', '需要', '資料', '字串', '數值', '對應'
}

PREFERRED_CJK_PHRASES = {
    '文字雲', 'hash', '雜湊', '碰撞', '資料結構', 'hash table', 'hash function',
    '記憶體', '固定長度', '加密演算法', '密碼驗證', '檔案完整性', '開放定址',
    'open addressing', 'double hashing', 'linked list', 'dictionary', 'python'
}
...
def tokenize_chinese_phrases(text):
    """簡單擷取 2~4 字中文片語，讓結果不只停留在單一中文字。"""
    counts = {}
    lowered_text = text.lower()

    # 先保留指定的重要詞
    for phrase in PREFERRED_CJK_PHRASES:
        cnt = lowered_text.count(phrase.lower())
        if cnt > 0:
            counts[phrase] = counts.get(phrase, 0) + cnt
        ...
        for n in (4, 3, 2):
            ...
            for i in range(len(run) - n + 1):
                ...
                if token in CHINESE_STOPWORDS:
                    continue
                if any(ch in '的是了和在就都而及與著或很也又被把讓跟並且如果所以但是因為' for ch in token[:1]):
                    continue
    ...
    for token, cnt in counts.items():
        if token in CHINESE_STOPWORDS:
            continue
        if token in PREFERRED_CJK_PHRASES or cnt >= 2:
...
def build_frequency_table(text):
    ...
    for token in tokenize_chinese_phrases(text):
        freq[token] = freq.get(token, 0) + 1
    ...
    for token, count in freq.items():
        if re.search(r'[A-Za-z]', token):
            ...
            if count >= 2 or token in PREFERRED_CJK_PHRASES:
                filtered[token] = count
